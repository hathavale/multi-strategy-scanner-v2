"""
Synthetic Short Strategy Implementation.

A Synthetic Short mimics shorting 100 shares of stock using options:
- Sell at-the-money (ATM) call option
- Buy at-the-money (ATM) put option

This creates a position with similar P/L profile to shorting stock but doesn't require borrowing shares.

Strategy Characteristics:
- Bearish directional bias
- 2 legs (short call + long put)
- Similar risk/reward to shorting stock
- Unlimited risk potential, substantial profit potential
- Net debit or small credit depending on strikes
"""
...
class SyntheticShortStrategy(BaseStrategy):
    """
    Synthetic Short strategy implementation.
    
    Entry Criteria:
    - Short call: ATM (delta 0.45-0.55), same strike as long put
    - Long put: ATM (delta -0.45 to -0.55), same strike as short call
    - Same expiration date for both legs
    - Total delta close to -1.0 (mimics shorting 100 shares)
    """
    
    def __init__(self):
        super().__init__(
            strategy_id='synthetic_short',
            strategy_name='synthetic_short',
            display_name='Synthetic Short',
            description='Short call + long put at same strike to mimic short stock position',
            num_legs=2,
    ...
    def get_default_parameters(self) -> Dict[str, Any]:
        """
        Get default parameters for Synthetic Short strategy.
    ...
    def validate_parameters(self, params: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        """
        Validate Synthetic Short parameters.
    ...
    def scan(self, symbol: str, filter_criteria: Dict[str, Any], 
        ...
        """
        Scan for Synthetic Short opportunities with pipeline tracking.
        ...
        - min_delta: Minimum combined delta magnitude (default: 0.90)
        ...
        for p in volume_filtered:
            ...
            if 0.40 <= call_delta <= 0.60 and -0.60 <= put_delta <= -0.40:
                delta_filtered.append(p)
        tracker.add_step('Delta Filter', 'ATM delta range (0.40-0.60)', len(volume_filtered), len(delta_filtered))
        ...
        for p in delta_filtered:
            ...
            combined_delta = abs(call_delta) + abs(put_delta)
            ...
            max_profit_value = strike - net_cost
            max_loss_value = 999999
            roi_estimate = ((strike * 0.10) / max(net_cost, 0.01)) * 100 if net_cost > 0 else 0
            
            legs_data = [
                {'type': 'call', 'position': 'short', 'strike': strike, 'expiry': expiry.isoformat(),
                 'premium': call_premium, 'delta': call_delta, 'volume': p['call']['volume'], 'dte': dte},
                {'type': 'put', 'position': 'long', 'strike': strike, 'expiry': expiry.isoformat(),
            ...
            opportunity = {
                ...
                'total_credit_debit': round(-net_cost, 2), 'max_profit': round(max_profit_value, 2),
                'max_loss': max_loss_value, 'breakeven_price': round(breakeven, 2),
                ...
                'metrics': {
                    ...
                    'max_profit': round(max_profit_value, 2), 'max_loss': max_loss_value,
        ...
        if synthetic_opportunities:
            ...
            print(f"\n✅ Found {len(synthetic_opportunities)} Synthetic Short opportunities")
            return synthetic_opportunities[:10]
        else:
            print(f"\n❌ No Synthetic Short opportunities found matching all criteria")
            return None
    
    def calculate_payoff(self, stock_prices: List[float], legs: List[Dict[str, Any]],
                        initial_cost: float) -> List[float]:
        """
        Calculate Synthetic Short payoff at expiration.
        
        At expiration:
        - Short call: -(max(stock_price - strike, 0) - call_premium)
        - Long put: max(strike - stock_price, 0) - put_premium
        
        Combined payoff moves -1:1 with stock price (profits when stock falls).
        """
        payoffs = []
...
if __name__ == "__main__":
    print("Synthetic Short Strategy Module")
    print("=" * 50)
    
    strategy = SyntheticShortStrategy()
    ...
    # Test payoff calculation (should be inverse linear like shorting stock)
    stock_prices = [80, 90, 100, 110, 120]
    test_legs = [
        {'type': 'call', 'position': 'short', 'strike': 100, 'premium': 3.00},
        {'type': 'put', 'position': 'long', 'strike': 100, 'premium': 2.80}
    ]
    
    payoffs = strategy.calculate_payoff(stock_prices, test_legs, -0.20)
    print("\n✓ Payoff calculation test (should be inverse linear):")
